...
param(
    ...
    [int]$EventLogCount          = 5,        # FIX: was hardcoded, now a parameter
    [string[]]$ExcludeServer     = @(),      # FIX: new parameter for maintenance windows
    [int]$KeepRunCount           = 10        # FIX: how many historical run files to keep
)
#endregion
...
# FIX: Apply ExcludeServer filter after loading
foreach ($group in $serverGroups.Keys) {
    $filtered = $serverGroups[$group] | Where-Object { $_ -notin $ExcludeServer }
    $serverGroups[$group] = [System.Collections.Generic.List[string]]@($filtered)
}
...
$checkServicesScript = {
    ...
    # FIX: Added event IDs 7024 and 7031 alongside 7034, and extended to 7-day window
    ...
    # FIX: Crash loop guard - skip auto-restart if >= 3 crashes in last 7 days
    function Invoke-AutoRestart {
        param([string]$ServiceName, [string]$Computer, $CimSession, [bool]$Enabled, [int]$RecentCrashes)
        if (-not $Enabled) { return $null }
        if ($RecentCrashes -is [int] -and $RecentCrashes -ge 3) {
            return "Auto-restart SKIPPED - $RecentCrashes crashes in last 7 days (possible crash loop)"
        }
        try {
    ...
    function Invoke-InformantChecks {
        ...
        foreach ($comp in $Components) {
            ...
            $jobs[$comp] = Start-Job -ScriptBlock {
                ...
                try {
                    # FIX: Ignore SSL errors for HTTPS Informant endpoints
    ...
    # FIX: CIM session with 2-attempt retry
    ...
    if (-not $session) {
        ...
        $out.Add(""); $out.Add("========================================")
        $out.Add("Zone     : $GroupName"); $out.Add("Server   : $ComputerName")
        $out.Add("  ERROR: Could not open CIM session after 2 attempts - $cimError")
        $out.Add("========================================")
        $instanceResults.Add([PSCustomObject]@{
            ComputerName = $ComputerName; GroupName = $GroupName
            InstanceLabel = "N/A"; Output = $out; CsvRows = $null
    ...
    $checkTime  = Get-Date -Format "yyyy-MM-dd HH:mm:ss"   # FIX: per-server check timestamp
    ...
    # FIX: Extended event log window to 7 days
    ...
    $out.Add(""); $out.Add("========================================")
    $out.Add("Zone     : $GroupName"); $out.Add("Server   : $ComputerName")
    $out.Add("  Checked      : $checkTime")   # FIX: per-server timestamp in console output
    ...
    if ($tomcatSvc) {
        # FIX: crash loop guard passed into Invoke-AutoRestart
        $tRecentCrashes = Get-ServiceRestartCount -ServiceName $tomcatSvc.Name -Computer $ComputerName
        ...
        $out.Add("  Auto-Restarts(7d):  $tRecentCrashes")   # FIX: label reflects 7-day window
        ...
        $csvRows.Add([PSCustomObject]@{
            ...
            RestartConfig = $tRestart;        AutoRestarts  = $tRecentCrashes
            ...
            AutoRestartNote = $autoNote       # FIX: capture auto-restart outcome in CSV
        })
    } else {
        $out.Add("  NOT FOUND")
    }
    ...
    if ($csSvcs) {
        foreach ($cs in $csSvcs) {
            # FIX: crash loop guard
            $csRecentCrashes = Get-ServiceRestartCount -ServiceName $cs.Name -Computer $ComputerName
            $autoNote        = Invoke-AutoRestart -ServiceName $cs.Name `
                -Computer $ComputerName -CimSession $session -Enabled $AutoRestartStopped `
                -RecentCrashes $csRecentCrashes
            if ($autoNote) { $out.Add("  [AUTO-RESTART] $autoNote") }
            ...
            $out.Add("  Auto-Restarts(7d):  $csRecentCrashes")  # FIX: label reflects 7-day window

            $csInformant = @{}
            if ($cs.State -eq "Running") {
                # FIX: TCP pre-check before firing HTTP requests
                $tcpOk     = $false
                $tcpClient = New-Object System.Net.Sockets.TcpClient
                try {
                    $tcpResult = $tcpClient.BeginConnect($ComputerName, 80, $null, $null)
                    $tcpOk     = $tcpResult.AsyncWaitHandle.WaitOne(($PortCheckTimeout * 1000), $false)
                } catch { } finally { $tcpClient.Close() }

                if (-not $tcpOk) {
                    # FIX: also try port 443 before giving up
                    $tcpClient2 = New-Object System.Net.Sockets.TcpClient
                    try {
                        $tcpResult2 = $tcpClient2.BeginConnect($ComputerName, 443, $null, $null)
                        $tcpOk      = $tcpResult2.AsyncWaitHandle.WaitOne(($PortCheckTimeout * 1000), $false)
                    } catch { } finally { $tcpClient2.Close() }
                }

                if (-not $tcpOk) {
                    $out.Add("  [WARN] TCP ports 80 and 443 both unreachable on $ComputerName - skipping Informant checks.")
                    if ($overallStatus -eq "OK") { $overallStatus = "WARN" }
                } else {
                    # FIX: detect HTTPS by checking port 443 availability
                    $csProto  = if ($tcpOk -and $tcpResult2 -ne $null -and
                                    $tcpResult2.AsyncWaitHandle.WaitOne(0,$false)) { "https" } else { "http" }
                    $pingBase = "${csProto}://$ComputerName/$($cs.Name)/cs?func=informant.ping"
            ...
            $csvRows.Add([PSCustomObject]@{
                ...
                RestartConfig   = $csRestart;    AutoRestarts  = $csRecentCrashes
    ...
    if ($csAdmin) {
        # FIX: crash loop guard
        $caRecentCrashes = Get-ServiceRestartCount -ServiceName $csAdmin.Name -Computer $ComputerName
        $autoNote        = Invoke-AutoRestart -ServiceName $csAdmin.Name `
            -Computer $ComputerName -CimSession $session -Enabled $AutoRestartStopped `
            -RecentCrashes $caRecentCrashes
        if ($autoNote) { $out.Add("  [AUTO-RESTART] $autoNote") }
        ...
        $out.Add("  Auto-Restarts(7d):  $caRecentCrashes")  # FIX: label reflects 7-day window
        ...
        $csvRows.Add([PSCustomObject]@{
            ...
            RestartConfig   = $caRestart;     AutoRestarts  = $caRecentCrashes
            WorkingSetMB    = "N/A";          ServerUptime  = $uptimeStr
            RecentReboot    = ($recentTag -ne ""); CpuPct   = $cpuAvg
            MemPct          = $memPct;        MemUsedGB     = $usedMemGB
            MemTotalGB      = $totalMemGB;    MemFreeGB     = $freeMemGB
    ...
    # FIX: Updated label to reflect 7-day window
    if ($eventLines.Count -gt 0) {
        $out.Add(""); $out.Add("Recent Application Log Events (last 7 days, Tomcat/CS related):")
    ...
    $instanceResults.Add([PSCustomObject]@{
        ...
        JobLog           = $jobLog;        CheckTime        = $checkTime   # FIX: pass through
    })
    return $instanceResults
}
#endregion
...
$htmlRows | Group-Object Zone | ForEach-Object {
    ...
    $zoneRows | Group-Object Server | ForEach-Object {
        ...
        $csRows | ForEach-Object {
            ...
            # FIX: GC tip count chip on instance row
            $gcTipChip = if ($serverResult -and $serverResult.GcRecommend -and
                             $serverResult.GcRecommend.Count -gt 0) {
                "<span class='chip warn'>$($serverResult.GcRecommend.Count) GC tips</span>"
            } else { "" }
            ...
            # FIX: Last Checked timestamp as first detail cell
            if ($serverResult -and $serverResult.CheckTime) {
                $detCells += "<div class='detail-cell'><div class='d-label'>Last Checked</div>" +
                             "<div class='d-value'>$(HtmlEncode $serverResult.CheckTime)</div></div>"
            }
            ...
            # FIX: Auto-restart outcome in detail panel
            if ($primary.AutoRestartNote) {
                $arCls = if ($primary.AutoRestartNote -match "SKIPPED|FAILED") { "warn-cell" } else { "ok-cell" }
                $detCells += "<div class='detail-cell $arCls full-width'>" +
                             "<div class='d-label'>Auto-Restart</div>" +
                             "<div class='d-value'>$(HtmlEncode $primary.AutoRestartNote)</div></div>"
            }
            ...
            # FIX: Updated label to 7 days
            $detCells += "<div class='detail-cell'><div class='d-label'>Auto-Restarts (7d)</div><div class='d-value'>$(HtmlEncode "$($primary.AutoRestarts)")</div></div>"
            ...
            if ($csAdminRow) {
                ...
                # FIX: CS Admin auto-restart outcome in detail panel
                if ($csAdminRow.AutoRestartNote) {
                    $arCls = if ($csAdminRow.AutoRestartNote -match "SKIPPED|FAILED") { "warn-cell" } else { "ok-cell" }
                    $detCells += "<div class='detail-cell $arCls full-width'>" +
                                 "<div class='d-label'>CS Admin Auto-Restart</div>" +
                                 "<div class='d-value'>$(HtmlEncode $csAdminRow.AutoRestartNote)</div></div>"
                }
            }
            if ($serverResult -and $serverResult.EventLines -and $serverResult.EventLines.Count -gt 0) {
                ...
                # FIX: Updated label to 7 days
                $detCells += "<div class='detail-cell warn-cell full-width'><div class='d-label'>Event Log (7 days)</div><div class='d-value'>$evText</div></div>"
            }
            ...
            if ($serverResult -and $serverResult.InformantResults -and
                ...
                $infToggleHtml = "<tr><td colspan='11'>" +
            ...
            <tr id='$detId' style='display:none'><td colspan='11' style='padding:0'>
        ...
        <tr class='server-header $serverStatus' onclick='toggleRow(this, &quot;$serverId&quot;)'>
          <td colspan='11'><div class='server-label'><span class='arrow'>v</span>$serverHeaderLabel</div></td>
        </tr>
          ...
          <tr><td colspan='11' style='padding:0'>
            <table class='inner-table'>
              <thead><tr>
                ...
                <th>CPU%</th><th>Mem%</th><th>Restarts(7d)</th><th>Overall</th><th>GC</th>
              </tr></thead>
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .detail-cell.ok-cell { background: #0d1f12; } .detail-cell.ok-cell .d-label { color: #3fb950; } .detail-cell.ok-cell .d-value { color: #3fb950; }
...
<table id='main-table'>
  <thead><tr>
    ...
    <th>Working Set</th><th>Heap Xms/Xmx</th><th>CPU%</th><th>Mem%</th>
    <th>Restarts(7d)</th><th>Overall</th><th>GC</th>
  </tr></thead>
...
# ── FIX: Old file cleanup - keep last N runs ───────────────────────────────────
foreach ($ext in @("*.log", "*.csv", "*.html")) {
    Get-ChildItem -Path $PSScriptRoot -Filter "ServiceCheck_$ext" -ErrorAction SilentlyContinue |
        Sort-Object Name -Descending |
        Select-Object -Skip $KeepRunCount |
        Remove-Item -Force -ErrorAction SilentlyContinue
}
Write-Log "Cleaned up old run files (kept last $KeepRunCount per type)" -Color Gray
...
if ($publishUrl) { Write-Log "URL  : $publishUrl" -Color Green }
#endregion

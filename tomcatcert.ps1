...
.DESCRIPTION
    ...
    executable path. After renewal exports cert, discovers IIS server from
    access logs, and remotely imports cert to IIS Trusted Root via WinRM.
    Supports pushing keystore to other SAN servers.
.PARAMETER WhatIf
...
function Get-IISServerFromLog {
    ...
    Write-Host ""
    $sel = Read-Host "Select IIS server (1-$([Math]::Min(5,$topIPs.Count)) or M)"
...
function Import-CertToIIS {
    ...
    try {
        if ($WhatIf) {
            Write-Host "  [WHATIF] Would import cert $thumbprint to Trusted Root on $iisServer" -ForegroundColor Magenta
            return $true
        }
        $params = @{
        ...
        Invoke-Command @params | ForEach-Object { Write-Log $_ -Level INFO }
...
# ============================================================
# PUSH KEYSTORE TO SAN SERVER VIA WINRM
# ============================================================
function Push-KeystoreToSanServer {
    ...
    $sb = {
        param($b64, $remotePath, $svcName, $bkTs, $isWhatIf)
        ...
        # Find service - by name first then description
        ...
        if (-not $svc) {
            ...
            if ($wmi) {
                $svc = Get-Service -Name $wmi.Name -ErrorAction SilentlyContinue
                Write-Output "  Found service via description: $($wmi.Name)"
            }
        }

        if ($svc -and -not $isWhatIf) {
            try {
                Restart-Service -Name $svc.Name -Force -ErrorAction Stop
                Write-Output "  Service '$($svc.Name)' restarted successfully"
            } catch {
                Write-Output "  WARNING: Could not restart '$($svc.Name)': $_"
            }
        } elseif ($svc -and $isWhatIf) {
            Write-Output "  [WHATIF] Would restart service '$($svc.Name)'"
        } else {
            Write-Output "  WARNING: Tomcat service not found - restart manually"
        }
    }
    ...
    try {
        ...
        if ($cred) { $params.Credential = $cred }
        Invoke-Command @params | ForEach-Object { Write-Log $_ -Level INFO -Server $targetServer }
        return $true
    } catch {
...
function Invoke-PostRenewal {
    ...
    # 1. Export cert - save to script directory for easy access
    ...
    if (-not $thumbprint) {
        ...
        if ($iisServer) {
            ...
            Import-CertToIIS -iisServer $iisServer -cerPath $cerPath -thumbprint $thumbprint -cred $cred
        } else {

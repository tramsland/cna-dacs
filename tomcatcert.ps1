...
<#
.SYNOPSIS
    Tomcat Keystore Management Script v5
.DESCRIPTION
    Manages Java keystores used by Tomcat. Auto-discovers keystore config
    from server.xml. After renewal, exports cert, discovers IIS server from
    access logs, and remotely imports cert to IIS Trusted Root via WinRM.
    Supports pushing keystore to other SAN servers and backing them up first.
.PARAMETER WhatIf
    Show what would be executed without making any changes.
.NOTES
    Requires PowerShell 5.1+, WinRM enabled on IIS and SAN servers.
#>
param([switch]$WhatIf)
...
$retentionDays = 90
$accessLogPath = "C:\Tomcat\logs"           # folder containing localhost_access_log.*
$accessLogPattern = "localhost_access_log*"
$iisLinesToScan   = 500                     # how many recent log lines to scan for IIS IP
...
# Restrict log file ACL to Administrators only
...
Set-Acl -Path $logFile -AclObject $acl -ErrorAction SilentlyContinue
...
function Find-Keytool {
    ...
    # 3. Check known customer Java location — scan one level deep under E:\customers\shared\dacs\java
    ...
    if (Test-Path $customerJavaRoot) {
        ...
        # Also check directly in the root (no version subfolder)
        $p = Join-Path $customerJavaRoot "bin\keytool.exe"
        if (Test-Path $p) { return $p }
    }

    # 5. Nothing found — prompt user
...
# ============================================================
# READ TOMCAT CONFIG FROM SERVER.XML
# ============================================================
function Get-TomcatKeystoreConfig {
    # 1. Find Tomcat service — by name/displayname first, then description
    ...
    # 2. Derive TomcatBase from service executable path
    $tomcatBase = $null
    $svcName    = $null
    if ($svc) {
        $svcName = $svc.Name
        ...
        if ($wmiSvc -and $wmiSvc.PathName) {
            # PathName is something like: E:\customers\shared\dacs\tomcat10\bin\tomcat10.exe
    ...
    if (-not $svc) {
        Write-Log "Service not found by name — searching service descriptions for 'Apache Tomcat'..." -Level WARN
    ...
    # Detect log path from server.xml or use configured default
    $logDir = $accessLogPath
    $logValve = $xml.Server.Service.Engine.Host.Valve |
                Where-Object { $_.className -match 'AccessLog' } |
                Select-Object -First 1
    if ($logValve -and $logValve.directory) {
        $resolved = $logValve.directory
        if (-not [System.IO.Path]::IsPathRooted($resolved)) {
            $resolved = Join-Path $tomcatBase $resolved
        }
        $logDir = $resolved
    }
...
function Invoke-Keytool {
    ...
    & $keytool @args 2>&1 | ForEach-Object { Write-Log $_ -Level DETAIL }
    return $LASTEXITCODE
}
...
function Get-CertSANs {
    ...
    foreach ($line in $certDetails) {
        ...
        if ($inSan) {
            if ($t.StartsWith("DNSName:"))  {
                $sanList.Add("dns:$(($t -split ':',2)[1].Trim())")
            }
            if ($t.StartsWith("IPAddress:")) {
                $sanList.Add("ip:$(($t -split ':',2)[1].Trim())")
            }
...
function Export-TomcatCert {
    param($keytool, $config, [string]$alias, [string]$exportPath)
    ...
    if ($rc -eq 0) {
        Write-Log "Cert exported to $exportPath" -Level INFO
        # Show thumbprint of exported cert
        $x509 = New-Object System.Security.Cryptography.X509Certificates.X509Certificate2($exportPath)
...
function Get-IISServerFromLog {
    ...
    # Return top IP (most frequent — likely the IIS ARR server)
    $topIP = ($ipCounts.GetEnumerator() | Sort-Object Value -Descending | Select-Object -First 5)
    Write-Host ""
    Write-Host "  Top IPs found in access logs:" -ForegroundColor Cyan
    $i = 1
    foreach ($entry in $topIP) {
        Write-Host "  [$i] $($entry.Key)  ($($entry.Value) hits)"
        $i++
    }
    Write-Host "  [M] Enter manually"
    Write-Host ""
    $sel = Read-Host "Select IIS server (1-$([Math]::Min(5,$topIP.Count)) or M)"
    if ($sel -eq 'M') {
        return Read-Host "Enter IIS server hostname or IP"
    }
    $idx = ($sel -as [int]) - 1
    if ($idx -ge 0 -and $idx -lt $topIP.Count) {
        return $topIP[$idx].Key
    }
...
# ============================================================
# IMPORT CERT TO IIS TRUSTED ROOT VIA WINRM
# ============================================================
function Import-CertToIIS {
    ...
    Write-Log "Connecting to IIS server $iisServer via WinRM..." -Level INFO
    ...
    $sb = {
        ...
        $tmpCer = "$env:TEMP\tomcat-new-$thumb.cer"
        [System.IO.File]::WriteAllBytes($tmpCer, $bytes)

        # Remove old cert(s) with same subject but different thumbprint
        $new = New-Object System.Security.Cryptography.X509Certificates.X509Certificate2($tmpCer)
        $store = New-Object System.Security.Cryptography.X509Certificates.X509Store(
            "Root","LocalMachine"
        )
        $store.Open("ReadWrite")
        $old = $store.Certificates | Where-Object {
            $_.Subject -eq $new.Subject -and $_.Thumbprint -ne $new.Thumbprint
        }
        foreach ($o in $old) {
            Write-Output "  Removing old cert: $($o.Thumbprint) (expires $($o.NotAfter))"
            $store.Remove($o)
        }
        # Import new cert
        $store.Add($new)
        ...
        Write-Output "  Imported cert $($new.Thumbprint) to Trusted Root on $env:COMPUTERNAME"
    }

    try {
        if ($WhatIf) {
            Write-Host "  [WHATIF] Would Invoke-Command on $iisServer to import cert thumbprint $thumbprint" -ForegroundColor Magenta
            return $true
        }
        $invokeParams = @{
            ComputerName = $iisServer
            ScriptBlock  = $sb
            ArgumentList = @($cerBase64, $thumbprint)
            ErrorAction  = "Stop"
        }
        if ($cred) { $invokeParams.Credential = $cred }
        $result = Invoke-Command @invokeParams
        $result | ForEach-Object { Write-Log $_ -Level INFO }
        Write-Log "Cert successfully imported to IIS Trusted Root on $iisServer" -Level INFO
        return $true
    } catch {
        Write-Log "Failed to import cert to IIS server ${iisServer}: $_" -Level ERROR
        return $false
    }
}
...
function Push-KeystoreToSanServer {
    ...
    Write-Log "Connecting to $targetServer to back up and update keystore..." -Level INFO
    $ksBytes  = [System.IO.File]::ReadAllBytes($localKeystorePath)
    $ksBase64 = [Convert]::ToBase64String($ksBytes)
    $bkTs     = Get-Date -Format 'yyyyMMdd-HHmmss'

    $sb = {
        param($b64, $remotePath, $svcName, $bkTs, $whatIf)
        $dir = Split-Path $remotePath
        if (-not (Test-Path $dir)) { New-Item $dir -ItemType Directory | Out-Null }

        # Backup existing
        if (Test-Path $remotePath) {
            $bkPath = "$remotePath.backup.$bkTs"
            Copy-Item $remotePath $bkPath -Force
            Write-Output "  Backed up to $bkPath"
        }
        ...
        if (-not $whatIf) {
            ...
            Write-Output "  Keystore updated at $remotePath"
        } else {
            Write-Output "  [WHATIF] Would write keystore to $remotePath"
        }

        # Restart service
        if ($svcName -and -not $whatIf) {
            # Try direct name first, fall back to description match
            $svc = Get-Service -Name $svcName -ErrorAction SilentlyContinue
            if (-not $svc) {
                Write-Output "  Service '$svcName' not found by name — searching descriptions..."
                $wmi = Get-WmiObject Win32_Service |
                       Where-Object {
                           $_.Name -match 'tomcat' -or
                           $_.DisplayName -match 'tomcat' -or
                           $_.Description -match 'Apache Tomcat'
                       } | Select-Object -First 1
                if ($wmi) {
                    $svc = Get-Service -Name $wmi.Name -ErrorAction SilentlyContinue
                    Write-Output "  Found service via description: $($wmi.Name) — $($wmi.DisplayName)"
                }
            }
            if ($svc) {
                try {
                    Restart-Service -Name $svc.Name -Force -ErrorAction Stop
                    Write-Output "  Service '$($svc.Name)' restarted successfully"
                } catch {
                    Write-Output "  WARNING: Could not restart service '$($svc.Name)': $_"
                }
            } else {
                Write-Output "  WARNING: Could not locate Tomcat service — restart manually"
            }
        } elseif ($svcName -and $whatIf) {
            Write-Output "  [WHATIF] Would restart service '$svcName'"
        }
    }
    ...
    try {
        $invokeParams = @{
            ...
            ScriptBlock  = $sb
            ArgumentList = @($ksBase64, $remoteKeystorePath, $remoteServiceName, $bkTs, $WhatIf.IsPresent)
            ErrorAction  = "Stop"
        }
        if ($cred) { $invokeParams.Credential = $cred }
        $result = Invoke-Command @invokeParams
        $result | ForEach-Object { Write-Log $_ -Level INFO -Server $targetServer }
        return $true
    } catch {
        Write-Log "Failed to push keystore to ${targetServer}: $_" -Level ERROR -Server $targetServer
        return $false
    }
}
...
function Invoke-TomcatRestart {
    ...
    if ($restart -ne 'y') {
        Write-Log "Tomcat restart skipped — restart manually to activate new cert." -Level WARN
        return
    }
    if ($WhatIf) {
...
# ============================================================
# COPY CERT TO SAN SERVER (top-level menu option X)
# ============================================================
function Invoke-CopyCertToSanServer {
    param($config, $keytool, $cred)
    $baseArgs = Get-BaseArgs $config
    ...
    Show-SanServerMenu -config $config -sanList $sanList -cred $cred -mode "copy"
}

# ============================================================
# SAN SERVER MENU (shared by renewal and copy flows)
# ============================================================
function Show-SanServerMenu {
    param($config, [System.Collections.Generic.List[string]]$sanList, $cred, [string]$mode)
    $localHost = $env:COMPUTERNAME.ToLower()
    # Build server list excluding current host
    $servers = @()
    foreach ($san in $sanList) {
        $host = ($san -replace '^dns:','').ToLower()
        if ($host -ne $localHost -and $host -notmatch '^\d') {
            $servers += $host
        }
    }
    if ($servers.Count -eq 0) {
        Write-Log "No remote SAN servers found (all SANs are local or IPs)." -Level WARN
        return
    }
    ...
    while ($true) {
        ...
        $target = $servers[$idx]
        $remoteKsPath = $config.KeystoreFile  # assume same path on remote
        $remoteSvcName = $config.ServiceName
        Push-KeystoreToSanServer `
            -targetServer $target `
            -localKeystorePath $config.KeystoreFile `
            -remoteKeystorePath $remoteKsPath `
            -remoteServiceName $remoteSvcName `
            -cred $cred
    }
}
...
# ============================================================
# POST-RENEWAL: EXPORT + IIS IMPORT + SAN PUSH
# ============================================================
function Invoke-PostRenewal {
    param($keytool, $config, [string]$alias, $cred)

    Write-Host ""
    Write-Host "  ── Post-Renewal Steps ──────────────────────" -ForegroundColor Cyan

    # 1. Export cert
    $cerName   = "$(Split-Path $config.KeystoreFile -LeafBase)-$(Get-Date -Format 'yyyyMMdd').cer"
    $cerPath   = Join-Path $storagePath $cerName
    $thumbprint = Export-TomcatCert $keytool $config $alias $cerPath
    if (-not $thumbprint) {
        Write-Log "Cert export failed — skipping IIS import." -Level ERROR
    } else {
        # 2. Find IIS server from access logs
        $iisServer = Get-IISServerFromLog -logDir $config.LogDir
        if ($iisServer) {
            Write-Log "IIS server identified as: $iisServer" -Level INFO
            $imported = Import-CertToIIS `
                -iisServer   $iisServer `
                -cerPath     $cerPath `
                -thumbprint  $thumbprint `
                -cred        $cred
            if ($imported) {
                Write-Log "IIS Trusted Root updated successfully." -Level INFO
            }
        } else {
            Write-Log "Could not determine IIS server — import cert manually from: $cerPath" -Level WARN
        }
    }
    ...
    if ($sanList.Count -gt 0) {
        ...
        if ($push -eq 'y') {
            Show-SanServerMenu -config $config -sanList $sanList -cred $cred -mode "push"
        }
    }
...
Write-Host "  Tomcat Keystore Management Script v5$(if($WhatIf){' [WHATIF]'})" -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not $keytool) {
    Write-Log "keytool.exe not found. Install a JDK or set JAVA_HOME." -Level ERROR; exit 1
}
...
# Prompt for shared credentials (used for IIS and SAN servers)
Write-Host "  Enter credentials for WinRM connections (IIS + SAN servers)." -ForegroundColor Cyan
$cred = Get-Credential -Message "WinRM credentials"
...
if ($choice -eq 'U') {
    ...
    if ($confirm -ne 'y') { Write-Log "Renewal aborted by user." -Level WARN; exit 0 }
    ...
    $newExpiry  = $newDetails | Where-Object { $_ -match "until:" }
    $newSANs    = Get-CertSANs $newDetails
    Write-Log "New expiry : $newExpiry" -Level INFO
    Write-Log "New SANs   : $($newSANs -join ', ')" -Level INFO

    Remove-OldBackups
    Invoke-PostRenewal -keytool $keytool -config $config -alias $aliasName -cred $cred
}
...
elseif ($choice -eq 'C') {
    ...
    $csrPath = Join-Path $config.TomcatBase "conf\$csrName"
    ...
    $rc = Invoke-Keytool $keytool $csrArgs
    if ($rc -eq 0) {
        Write-Log "CSR created at $csrPath" -Level INFO
        Write-Host "  Submit this CSR to your CA. When you receive the signed cert, use option [I] to import." -ForegroundColor Cyan
    } else {
...
elseif ($choice -eq 'I') {
    ...
    $newExpiry  = $newDetails | Where-Object { $_ -match "until:" }
    $newSANs    = Get-CertSANs $newDetails
    Write-Log "Expiry : $newExpiry" -Level INFO
    Write-Log "SANs   : $($newSANs -join ', ')" -Level INFO
...
elseif ($choice -eq 'X') {
    ...
    Invoke-CopyCertToSanServer -config $config -keytool $keytool -cred $cred
}
...
,''  # strip any args
            $tomcatBase = Split-Path (Split-Path $exePath)  # up two levels from bin\exe
            Write-Log "Tomcat base from service: $tomcatBase" -Level INFO
        }
    }

    # 3. Fall back to known fixed paths if service path not resolved
    if (-not $tomcatBase -or -not (Test-Path $tomcatBase)) {
        ...
        foreach ($pattern in $knownBases) {
            $found = Get-Item $pattern -ErrorAction SilentlyContinue | Select-Object -First 1
            if ($found -and (Test-Path (Join-Path $found.FullName "conf\server.xml"))) {
                $tomcatBase = $found.FullName
    ...
    # 4. Manual entry if still not found
    if (-not $tomcatBase) {
